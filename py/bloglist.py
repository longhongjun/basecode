...
def get_page_inner_url(url):
    ...
    lParser.close()
    for name in lParser.urls:
        urls.append(name)
    lParser=""
    
    
cat_list=['arch','database','geek','jobs','mylife','opensource','review','security','sitelog','startup','tech-memo','web']
cat_list=['arch']

user_url="http://dbanotes.net/"
filename = "./blog1.txt"
for cat_name in cat_list:
    cat_url=user_url+cat_name+'/page/'
    filename='./'+cat_name+'.txt'
    for i in range(1,2):
        get_url=cat_url+str(i)
        content = get_page_inner_url(get_url)
    for url in urls:
        content_list = some_url(url)
        write_file(filename, '-----\r\n')
